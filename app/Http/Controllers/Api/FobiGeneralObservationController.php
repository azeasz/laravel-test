...
class FobiGeneralObservationController extends Controller
{
    ...
    private function detectRankFromName($scientificName)
    {
        ...
        if ($count == 1) {
            // Default untuk satu kata adalah GENUS, tapi perlu verifikasi lebih lanjut
            // karena CLASS atau GROUP juga bisa satu kata
            return 'GENUS';
        } else if ($count == 2) {
            ...
            return 'VARIETY';
        } else if ($count >= 3 && (strpos(strtolower($scientificName), 'f.') !== false || 
                                 strpos(strtolower($scientificName), 'forma') !== false)) {
            return 'FORM';
        } else if ($count >= 3) {
            // Tiga kata atau lebih tanpa indikator yang jelas, kemungkinan subspecies
            return 'SUBSPECIES';
        }
