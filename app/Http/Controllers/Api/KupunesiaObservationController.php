...
class KupunesiaObservationController extends Controller
{
    ...
    public function addIdentification(Request $request, $id)
    {
        try {
            ...
            // Insert identifikasi baru sebagai identifikasi utama (is_main = true)
    ...
    public function agreeWithIdentification($observationId, $identificationId)
    {
        try {
            ...
            // Ambil data identifikasi yang disetujui
            $agreedIdentification = DB::table('kupunesia_identifications')
                ->where('id', $identificationId)
                ->where('is_main', true) // Pastikan ini identifikasi utama
                ->first();

            if (!$agreedIdentification) {
                return response()->json([
                    'success' => false,
            ...
            // Insert sebagai sub-identifikasi (is_main = false)
    ...
    public function disagreeWithIdentification(Request $request, $observationId, $identificationId)
    {
        try {
            ...
            // Ambil data identifikasi yang ditolak
            $disagreedIdentification = DB::table('kupunesia_identifications')
                ->where('id', $identificationId)
                ->where('is_main', true) // Pastikan ini identifikasi utama
                ->first();

            if (!$disagreedIdentification) {
                return response()->json([
                    'success' => false,
                    'message' => 'Identifikasi tidak ditemukan'
                ], 404);
            ...
            // Insert sebagai identifikasi utama baru (is_main = true)
            DB::table('kupunesia_identifications')->insert([
                'observation_id' => $observationId,
                'user_id' => auth()->id(),
                ...
                'identification_level' => $request->identification_level,
                'is_agreed' => false,
                'is_main' => true, // Set sebagai identifikasi utama baru
                'observation_type' => 'kupunesia',
                'comment' => $request->comment,
                'photo_path' => $photoPath,
                'created_at' => now(),
