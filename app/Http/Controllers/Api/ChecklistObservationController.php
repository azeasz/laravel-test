...
class ChecklistObservationController extends Controller
{
    ...
    public function searchTaxa(Request $request)
    {
        try {
            ...
            Log::info("Searching taxa with query: {$query}, page: {$page}, per_page: {$perPage}, source: {$source}, rank: {$rank}");
            ...
            Log::info("Found {$total} taxa results, returning page {$page} with {$results->count()} items");
    ...
    private function getIdentificationsWithPhotos($id, $source = null)
    {
        ...
        $query = DB::table('taxa_identifications as i')
            ...
            ->select([
                ...
                't.subform',
                't.scientific_name',
                // Ambil semua field common name untuk semua level taksonomi
                't.cname_species',
                't.cname_genus',
                't.cname_family',
                't.cname_order',
