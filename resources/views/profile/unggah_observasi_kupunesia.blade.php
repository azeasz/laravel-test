...
<head>
    ...
    <title>Unggah Observasi Checklist Kupunesia</title>
...
<body>
    ...
    <div class="container mt-5">
        <div class="row">
            ...
            <div class="col-md-9">
                <div class="content">
                    <h5>Unggah Observasi Checklist Kupunesia</h5>
                    <form action="{{ route('fobi.upload.store_kupunesia') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="type" value="kupunesia">
                            ...
                            <script>
                                document.addEventListener('DOMContentLoaded', function() {
                                    ...
                                    document.getElementById('save-location').addEventListener('click', function() {
                                        ...
                                        locationInput.value = latInput + ', ' + lngInput + ', ' + accInput + ', ' + localityNotesInput; // Format: "lat, lng, acc, locality notes, geoprivacy"
                        ...
                        <div class="upload-section">
                            ...
                            <table class="table">
                                ...
                                <tbody id="butterfly-list">
                                    <!-- Daftar kupu-kupu akan ditambahkan di sini -->
                                </tbody>
                            </table>
                            <button type="button" class="btn" id="add-butterfly">Tambah Jenis</button>
                        </div>

                        <!-- Modal untuk Tambah Jenis Kupu-Kupu -->
                        <div class="modal fade" id="addButterflyModal" tabindex="-1" role="dialog" aria-labelledby="addButterflyModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="addButterflyModalLabel">Tambah Jenis Kupu-Kupu</h5>
                                        <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body">
                                        <div class="form-group">
                                            <label for="butterfly_species">Jenis Kupu-Kupu</label>
                                            <input type="text" name="butterfly_species" id="butterfly_species" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="butterfly_count">Jumlah Individu</label>
                                            <input type="number" name="butterfly_count" id="butterfly_count" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="butterfly_notes">Catatan</label>
                                            <textarea name="butterfly_notes" id="butterfly_notes" rows="2"></textarea>
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                                        <button type="button" class="btn btn-primary" id="save-butterfly">Simpan</button>
                                    </div>
                                </div>
                            </div>
                        </div>

<!-- Modal untuk Edit Jenis Kupu-Kupu -->
<div class="modal fade" id="editButterflyModal" tabindex="-1" role="dialog" aria-labelledby="editButterflyModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editButterflyModalLabel">Edit Jenis Kupu-Kupu</h5>
                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <div class="form-group">
                    <label for="edit_butterfly_species">Jenis Kupu-Kupu</label>
                    <input type="text" name="edit_butterfly_species" id="edit_butterfly_species">
                </div>
                <div class="form-group">
                    <label for="edit_butterfly_count">Jumlah Individu</label>
                    <input type="number" name="edit_butterfly_count" id="edit_butterfly_count">
                </div>
                <div class="form-group">
                    <label for="edit_butterfly_notes">Catatan</label>
                    <textarea name="edit_butterfly_notes" id="edit_butterfly_notes" rows="2"></textarea>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                <button type="button" class="btn btn-primary" id="update-butterfly">Simpan</button>
            </div>
        </div>
    </div>
</div>
                        ...
                        <div class="modal fade" id="addPhotoModal" tabindex="-1" role="dialog" aria-labelledby="addPhotoModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="form-group">
                                            <label for="butterfly_photo">Unggah Foto</label>
                                            <input type="file" class="form-control" id="butterfly_photo" accept="image/*">
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                        ...
                        <script>
                            document.getElementById('add-butterfly').addEventListener('click', function() {
                                $('#addButterflyModal').modal('show');
                            });

                            document.getElementById('save-butterfly').addEventListener('click', function() {
                                var butterflyList = document.getElementById('butterfly-list');
                                var butterflySpecies = document.getElementById('butterfly_species').value;
                                var butterflyCount = document.getElementById('butterfly_count').value;
                                var butterflyNotes = document.getElementById('butterfly_notes').value;

                                var butterflyItem = document.createElement('tr');
                                butterflyItem.classList.add('butterfly-item');
                                butterflyItem.innerHTML = `
                                    <td>
                                    </td>
                                    <td>${butterflySpecies}</td>
                                    <td>${butterflyCount}</td>
                                    <td>${butterflyNotes}</td>
                                    <td>
                                        <button type="button" class="btn btn-primary btn-edit-butterfly">Edit</button>
                                        <button type="button" class="btn btn-danger btn-remove-butterfly">Hapus</button>
                                        <button type="button" class="btn btn-primary btn-add-photo">Tambah Foto</button>
                                    </td>
                                `;
                                butterflyList.appendChild(butterflyItem);

                                $('#addButterflyModal').modal('hide');
                            });

                            document.getElementById('butterfly-list').addEventListener('click', function(event) {
    if (event.target.classList.contains('btn-remove-butterfly')) {
        event.target.parentElement.parentElement.remove();
    } else if (event.target.classList.contains('btn-edit-butterfly')) {
        var currentRow = event.target.parentElement.parentElement;
        document.getElementById('edit_butterfly_species').value = currentRow.cells[1].innerText;
        document.getElementById('edit_butterfly_count').value = currentRow.cells[2].innerText;
        document.getElementById('edit_butterfly_notes').value = currentRow.cells[3].innerText;
        $('#editButterflyModal').data('currentRow', currentRow).modal('show');
...
document.getElementById('update-butterfly').addEventListener('click', function() {
    var currentRow = $('#editButterflyModal').data('currentRow');
    currentRow.cells[1].innerText = document.getElementById('edit_butterfly_species').value;
    currentRow.cells[2].innerText = document.getElementById('edit_butterfly_count').value;
    currentRow.cells[3].innerText = document.getElementById('edit_butterfly_notes').value;
    $('#editButterflyModal').modal('hide');
});
                            document.getElementById('save-photo').addEventListener('click', function() {
                                var currentRow = $('#addPhotoModal').data('currentRow');
                                var photoInput = document.getElementById('butterfly_photo');
