...
<head>
    ...
    <title>Observasi {{ $user->uname }}</title>
    ...
    <div class="container mt-5">
        <div class="row">
                ...
                <div class="profile-sidebar">
                    ...
                    <div class="profile-usermenu">
                        <ul class="nav flex-column">
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('profile.public', $user->uname) }}">
                                    Profil
                                </a>
                            </li>
                            <li class="nav-item active">
                                <a class="nav-link" href="{{ route('profile.public.observasi', $user->uname) }}">
                                    Observasi
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="#">
                                    Spesies <span class="badge bg-secondary">200</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="#">
                                    Identifikasi <span class="badge bg-secondary">12</span>
                                </a>
                            </li>
                        </ul>
                    </div>
            ...
            <div class="col-md-9 mt-3">
                <h5 style="border-bottom: 1px solid #000000; padding-bottom: 10px;">Observasi oleh {{ $user->uname }}</h5>
                <div id="map" style="height: 400px; width: 100%;"></div>
                <div class="content mt-3">
                    @foreach($paginatedObservations as $observation)
                        <div class="card mb-3">
                            {{-- <img src="{{ asset('storage/observations/' . $observation->image) }}" alt="Observation Image" class="card-img-top"> --}}
                            <div class="card-body">
                                {{-- <h5 class="card-title">{{ $observation->name }}</h5> --}}
                                <p class="card-text">{{ $observation->fauna_name }}</p>
                                <p class="card-text">{{ $observation->fauna_family }}</p>
                                <p class="card-text" id="location-{{ $observation->id }}"></p>
                            </div>
                    ...
                    @endforeach
                    <div class="paginasi justify-content-center mt-2 mb-3">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @foreach($observations as $observation)
                var marker = L.marker([{{ $observation->latitude }}, {{ $observation->longitude }}])
