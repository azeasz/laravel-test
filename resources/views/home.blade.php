...
<head>
    ...
    justify-content: center;
    padding: 10px;
    background-color: #679995; /* Sesuaikan dengan warna sidebar */
}
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Simpan nilai awal statistik
        const initialStats = {
            burungnesiaCount: {{ $burungnesiaCount }},
            kupunesiaCount: {{ $kupunesiaCount }},
            observasiCount: {{ $burungnesiaCount + $kupunesiaCount }},
            fotoAudioCount: 3987,
            spesiesCount: 1345,
            kontributorCount: 678
        };
        ...
        mapViewIcon.addEventListener('click', function() {
            ...
            if (!map) { // Check if map is already initialized
                ...
                L.control.scale({
                    ...
                    imperial: false, // Nonaktifkan satuan imperial jika tidak diperlukan
                }).addTo(map);
                ...
                @foreach($checklists as $checklist)
                    ...
                    checklists.push({
                        image: "{{ asset('storage/icon/birur.jpeg') }}",
                        id: {{ $checklist->id }},
    ...
    function renderPage(page) {
        ...
        const pageItems = filteredChecklists.slice(startIndex, endIndex);

        // Add new content
        const promises = pageItems.map(checklist => {
            const checklistElement = document.createElement('div');
            const imageUrl = checklist.image ? `<img src="${checklist.image}" alt="Image" style="width: 50px; height: auto; margin-right: 10px;">` : '';
            const logo = checklist.source === 'burungnesia' ? '<img src="{{ asset('storage/icon/icon.png') }}" alt="Burungnesia" style="width: 50px; height: auto; margin-right: 10px;">' : '<img src="{{ asset('storage/icon/kupnes.png') }}" alt="Kupunesia" style="width: 50px; height: auto; margin-right: 10px;">';
            const checklistId = checklist.id;
            checklistElement.innerHTML = `
                <div style="display: flex; align-items: center;">
                    ${checklist.source === 'burungnesia' || checklist.source === 'kupunesia' ? logo : imageUrl}
                    <div>
            ...
            const logoElement = checklistElement.querySelector('img[alt="Burungnesia"], img[alt="Kupunesia"]');
        if (logoElement) {
            logoElement.addEventListener('click', () => {
                window.open(`/checklist/${checklist.source}`, '_blank');
            });
        }


            // Fetch species data with pagination
            return fetch(`/api/species/${checklistId}?page=${page}&per_page=${itemsPerPage}`)
                .then(response => response.json())
                .then(data => {
                    // Pastikan data.burungnesia dan data.kupunesia ada dan merupakan array
                    if (Array.isArray(data.burungnesia)) {
                        data.burungnesia.map(species => {
                            const speciesElement = document.createElement('div');
                            speciesElement.innerHTML = `
                                <div style="display: flex; align-items: center;">
                                    ${checklist.source === 'burungnesia' ? logo : imageUrl}
                                    <div>
                                        <p><strong>ID:</strong> ${species.id}</p>
                                        <p><strong>Spesies:</strong> ${species.nameLat}<br><small>${species.nameId}</small></p>
                                        <p><strong>Lokasi:</strong> ${species.latitude}, ${species.longitude}</p>
                                    </div>
                                </div>
                            `;
                            sidebarContent.appendChild(speciesElement);
                            const logoElement = speciesElement.querySelector('img');
                            if (logoElement) {
                                logoElement.addEventListener('click', () => {
                                    window.open(`/species/${checklist.id}/${species.id}`, '_blank');
                                });
                            }
                        });
                    }

                    if (Array.isArray(data.kupunesia)) {
                        data.kupunesia.map(species => {
                            const speciesElement = document.createElement('div');
                            speciesElement.innerHTML = `
                                <div style="display: flex; align-items: center;">
                                    ${checklist.source === 'kupunesia' ? logo : imageUrl}
                                    <div>
                                        <p><strong>ID:</strong> ${species.id}</p>
                                        <p><strong>Spesies:</strong> ${species.nameLat}<br><small>${species.nameId}</small></p>
                                        <p><strong>Lokasi:</strong> ${species.latitude}, ${species.longitude}</p>
        ...
        // Wait for all promises to resolve before adding pagination controls
        Promise.all(promises).then(() => {
            const totalPages = Math.ceil(filteredChecklists.length / itemsPerPage);

            // Update button visibility
            document.getElementById('prev-button').style.display = page > 1 ? 'block' : 'none';
            document.getElementById('next-button').style.display = page < totalPages ? 'block' : 'none';
        });
    }
...
document.getElementById('next-button').addEventListener('click', () => {
    ...
    if (currentPage < totalPages) {
        ...
        renderPage(currentPage);
                    ...
                    // Update statistics
                    const burungnesiaCount = filteredChecklists.filter(checklist => checklist.source === 'burungnesia').length;
                                    const kupunesiaCount = filteredChecklists.filter(checklist => checklist.source === 'kupunesia').length;
                                    const observasiCount = filteredChecklists.length;
                                    const fotoAudioCount = filteredChecklists.filter(checklist => checklist.source === 'foto_audio').length;
                                    const spesiesCount = new Set(filteredChecklists.map(checklist => checklist.species)).size;
                                    const kontributorCount = new Set(filteredChecklists.map(checklist => checklist.contributor)).size;

                                    const burungnesiaElement = document.getElementById('burungnesia-count');
                                    const kupunesiaElement = document.getElementById('kupunesia-count');
                                    const observasiElement = document.getElementById('observasi-count');
                                    const fotoAudioElement = document.getElementById('foto-audio-count');
                                    const spesiesElement = document.getElementById('spesies-count');
                                    const kontributorElement = document.getElementById('kontributor-count');

                                    if (burungnesiaElement) burungnesiaElement.textContent = burungnesiaCount.toLocaleString('id-ID');
                                    if (kupunesiaElement) kupunesiaElement.textContent = kupunesiaCount.toLocaleString('id-ID');
                                    if (observasiElement) observasiElement.textContent = observasiCount.toLocaleString('id-ID');
                                    if (fotoAudioElement) fotoAudioElement.textContent = fotoAudioCount.toLocaleString('id-ID');
                                    if (spesiesElement) spesiesElement.textContent = spesiesCount.toLocaleString('id-ID');
                                    if (kontributorElement) kontributorElement.textContent = kontributorCount.toLocaleString('id-ID');
                                }
                                ...
                                // Event listener untuk mengembalikan statistik ke nilai awal saat mengklik di luar peta
                                ...
                                function resetStats() {
                                    document.getElementById('burungnesia-count').textContent = initialStats.burungnesiaCount.toLocaleString('id-ID');
                                    document.getElementById('kupunesia-count').textContent = initialStats.kupunesiaCount.toLocaleString('id-ID');
                                    document.getElementById('observasi-count').textContent = initialStats.observasiCount.toLocaleString('id-ID');
                                    document.getElementById('foto-audio-count').textContent = initialStats.fotoAudioCount.toLocaleString('id-ID');
                                    document.getElementById('spesies-count').textContent = initialStats.spesiesCount.toLocaleString('id-ID');
                                    document.getElementById('kontributor-count').textContent = initialStats.kontributorCount.toLocaleString('id-ID');
                                }
...
<body>
    ...
    <nav>
      ...
      <div class="user-info">
        ...
        <span><strong style="font-size: 18px;">{{ $userTotalObservations }}</strong><br><small>Observasi</small></span>
    ...
    <!-- Small Screen Navigation Bar -->
      ...
      <script>
        ...
        <div class="d-flex justify-content-between align-items-center p-2 w-100" style="background-color: #679995;">
            ...
            @php
            $burungnesiaCount = number_format(DB::connection('second')->table('checklist_fauna')->count(), 0, ',', '.');
            $kupunesiaCount = number_format(DB::connection('third')->table('checklist_fauna')->count(), 0, ',', '.');
            @endphp
<div class="stats row align-items-center text-center w-100">
    <div class="col-md-2 col-6 mb-2"><strong class="text-md-left text-warning" style="font-size: 14px;" id="observasi-count">{{$burungnesiaCount + $kupunesiaCount}}</strong> <br> <small style="font-size: 12px;">OBSERVASI</small></div>
    ...
    <div class="col-md-2 col-6 mb-2"><strong class="text-md-left text-warning" style="font-size: 14px;" id="kupunesia-count">{{ $kupunesiaCount }}</strong> <br> <small style="font-size: 12px;">KUPUNESIA</small></div>
            ...
            <div id="map">
                <div id="sidebar" class="sidebar">
                    ...
                    <div id="sidebar-header" class="sidebar-header">
                        <button id="prev-button" class="arrow-button up"><i class="fa fa-chevron-up"></i></button>
                    </div>
                    <div id="sidebar-content" class="sidebar-content">
                        <!-- Konten sidebar akan ditambahkan di sini melalui JavaScript -->
                    </div>
                    <div id="sidebar-footer" class="sidebar-footer">
                        <button id="next-button" class="arrow-button down"><i class="fa fa-chevron-down"></i></button>
                    </div>
                </div>
            </div>
 ...
 <div class="tree-view" style="display: none;">
    <!-- Tambahkan tombol untuk membuka dan menutup semua -->
    <div class="bulk-controls">
        <button id="open-all"><i class="fa fa-plus" aria-hidden="true"></i></button>
        <button id="close-all"><i class="fa fa-minus" aria-hidden="true"></i></button>
    </div>
    <ul>
        <li>
            <details>
                ...
                <ul>
                    <li>
                        <details>
                            <summary>Animalia <span class="category-text text-muted"><small>(Kingdom)</small></span></summary>
                            <ul>
                                @foreach($taxontest->where('kingdom', 'Animalia')->groupBy('phylum') as $phylum => $animalsByPhylum)
                                @if($phylum)
                                <li>
                                    <details>
                                        <summary>{{ $phylum }} <span class="category-text text-muted"><small>(Phylum)</small></span></summary>
                                        <ul>
                                            @if($phylum === 'Arthropoda')
                                            <li>
                                                <details>
                                                    <summary>Insecta <span class="category-text text-muted"><small>(Class)</small></span></summary>
                                                    <ul>
                                                        <li>
                                                            <details>
                                                                <summary>Lepidoptera <span class="category-text text-muted"><small>(Order)</small></span></summary>
                                                                <ul>
                                                                    @php
                                                                    $lepidopteraFaunas = DB::connection('third')->table('faunas')
                                                                        ->get();
                                                                    $lepidopteraByFamily = $lepidopteraFaunas->groupBy('family');
                                                                    @endphp

                                                                    @foreach($lepidopteraByFamily as $family => $faunaGroup)
                                                                    <li>
                                                                        <details>
                                                                            ...
                                                                            <ul>
                                                                                ...
                                                                                <li>
                                                                                    <a href="#"><i>{{ $fauna->nameLat }}</i> <span class="category-text text-muted" style="font-size: 12px;">{{ $fauna->nameId }}</span> <span class="category-text text-muted"><small>(Species)</small></span></a>
                                                                                </li>
                                                                                @endforeach
                                                                            </ul>
                                                                        </details>
                                            ...
                                            @if($class)
                                                <li>
                                                    <details>
                                                        <summary>{{ $class }} <span class="category-text text-muted"><small>(Class)</small></span></summary>
                                                        <ul>
                                                            @foreach($animalsByClass->groupBy('order')->sortBy('order_key') as $order => $animalsByOrder)
                                                            @if($order)
                                                            <li>
                                                                <details>
                                                                    <summary>{{ $order }} <span class="category-text text-muted"><small>(Order)</small></span> ({{ $animalsByOrder->count() }})</summary>
                                                                    <ul>
                                                                                                ...
                                                                                                <details>
                                                                                                    <summary>
                                                                                                        <a href="{{ route('genus.gallery', ['genus' => $genus]) }}">
                                                                                                            {{ $genus }} <span class="category-text text-muted"><small>(Genus)</small></span> ({{ $animalsByGenus->count() }})
                                                                                                        </a>
                                                                                                    </summary>
                                                                                                    <ul>
                                                                                                        @foreach($animalsByGenus as $animal)
                                                                                                        @if($animal->cnameSpecies && $animal->species)
                                                                                                            <li><i>{{ $animal->species }}</i> <span class="category-text text-muted" style="font-size: 12px;">{{ $animal->cnameSpecies }}</span> <span class="category-text text-muted"><small>(Species)</small></span></li>
                                                                                                        @endif
                                                                                                        @endforeach
                                                                                                    </ul>
                                                                                                </details>
                                ...
                                <li>
                                    <details>
                                        <summary>Chordata <span class="category-text text-muted"><small>(Phylum)</small></span></summary>
                                        <ul>
                                            <li>
                                                <details>
                                                                            ...
                                                                            <li>
                                                                                <details>
                                                                                    ...
                                                                                    <ul>
                                                                                        ...
                                                                                        @foreach($genusData as $genus => $genusFaunas) <!-- Iterasi genus yang diambil -->
                                                                                            <li>
                                                                                                <details>
                                                                                                    <summary>
                                                                                                        <a href="{{ route('genus.gallery', ['genus' => $genus]) }}">
                                                                                                    ...
                                                                                                    <ul>
                                                                                                        @foreach($genusFaunas as $g) <!-- Iterasi spesies dalam genus -->
                                                                                                            <li>
                    ...
                    <li>
                        <details>
                            <summary>Plantae <span class="category-text text-muted"><small>(Kingdom)</small></span></summary>
                            <ul>
                                                ...
                                                @if($class)
                                                    <li>
                                                        <details>
                                                            <summary>{{ $class }} <span class="category-text text-muted"><small>(Class)</small></span></summary>
                                                            <ul>
                                                                @foreach($plantsByClass->groupBy('order')->sortBy('order_key') as $order => $plantsByOrder)
                                                                @if($order)
                                                <li>
                                                    <details>
                                                        <summary>{{ $order }} <span class="category-text text-muted"><small>(Order)</small></span> ({{ $plantsByOrder->count() }})</summary>
                                                        <ul>
                                                                            ...
                                                                            @if($genus)
                                                                                <li>
                                                                                    <details>
                                                                                        <summary>
                                                                                            <a href="{{ route('genus.gallery', ['genus' => $genus]) }}">
                                                                                                {{ $genus }} <span class="category-text text-muted"><small>(Genus)</small></span> ({{ $plantsByGenus->count() }})
                                                                                            </a>
                                                                                        </summary>
                                                                                        <ul>
                                                                                            @foreach($plantsByGenus as $plant)
                                                                                            @if($plant->cnameSpecies && $plant->species)
                                                                                                <li><i>{{ $plant->species }}</i> <span class="category-text text-muted" style="font-size: 12px;">{{ $plant->cnameSpecies }}</span> <span class="category-text text-muted"><small>(Species)</small></span></li>
                                                                                            @endif
                                                                                            @endforeach
                                                                                        </ul>
                                                                                    </details>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const detailsElements = document.querySelectorAll('.tree-view details');
